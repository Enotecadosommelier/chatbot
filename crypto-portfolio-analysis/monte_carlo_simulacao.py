...
assets = ["BTC","ETH","BNB","SOL","XRP","LINK","AVAX","AAVE","SUI","USDT"]
weights = np.array([0.18,0.15,0.10,0.09,0.07,0.06,0.05,0.03,0.02,0.25], dtype=np.float32)
assert abs(weights.sum()-1) < 1e-6

mu    = np.array([0.15,0.18,0.12,0.25,0.10,0.20,0.20,0.18,0.22,0.04], dtype=np.float32)
sigma = np.array([0.50,0.65,0.55,0.85,0.70,0.75,0.90,0.80,1.00,0.005], dtype=np.float32)

n = len(assets)
corr = np.array([
   [1.00, 0.85, 0.75, 0.70, 0.55, 0.65, 0.68, 0.60, 0.60, 0.00],
   [0.85, 1.00, 0.70, 0.75, 0.50, 0.75, 0.72, 0.70, 0.65, 0.00],
   [0.75, 0.70, 1.00, 0.65, 0.50, 0.60, 0.60, 0.55, 0.55, 0.00],
   [0.70, 0.75, 0.65, 1.00, 0.50, 0.65, 0.75, 0.60, 0.70, 0.00],
   [0.55, 0.50, 0.50, 0.50, 1.00, 0.50, 0.45, 0.40, 0.45, 0.00],
   [0.65, 0.75, 0.60, 0.65, 0.50, 1.00, 0.65, 0.65, 0.60, 0.00],
   [0.68, 0.72, 0.60, 0.75, 0.45, 0.65, 1.00, 0.65, 0.70, 0.00],
   [0.60, 0.70, 0.55, 0.60, 0.40, 0.65, 0.65, 1.00, 0.60, 0.00],
   [0.60, 0.65, 0.55, 0.70, 0.45, 0.60, 0.70, 0.60, 1.00, 0.00],
